...
export default function MerchantOnboardingScreen() {
  ...
  const fetchMerchantData = async (token: string): Promise<any> => {
    ...
    try {
      ...
      console.log(
        '🌐 Calling status API endpoint: http://172.20.10.12:5000/api/v0/payout-portal/wallet-setup/sellers/status',
      );

      const response = await fetch(
        'http://172.20.10.12:5000/api/v0/payout-portal/wallet-setup/sellers/status',
        {
  ...
  const handleSubmitToBackend = async (formData: FormData): Promise<any> => {
    ...
    try {
      ...
      const response = await fetch(
        'http://172.20.10.12:5000/api/v0/payout-portal/wallet-setup/sellers',
        {
          method: 'POST',
