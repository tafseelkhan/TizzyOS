...
export const signup = async ({ identifier, role }: SignupPayload) => {
  ...
  try {
    const response = await fetch('http://172.20.10.12:5000/api/auth/signup', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
export const verifySignup = async ({
  ...
  try {
    const response = await fetch('http://172.20.10.12:5000/api/auth/verify-signup', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
export const login = async ({ identifier }: LoginPayload) => {
  ...
  return await fetchHandler(
    'http://172.20.10.12:5000/api/auth/send-login-otp',
    {
      method: 'POST',
...
export const removeLinkedAccount = async (targetUserId: string) => {
  ...
  const data = await fetchHandler(
    'http://172.20.10.12:5000/api/auth/remove-linked-account',
    {
      method: 'DELETE',
      headers: {
        Authorization: `Bearer ${token}`,
...
export const checkAuthStatus = async () => {
  try {
    ...
    const response = await axios.get(
      'http://172.20.10.12:5000/api/auth/check-auth',
      {
