...
export const verifyToken = async (
  ...
  try {
    ...
    const res = await axios.get('http://10.171.84.121:5000/api/v0/auth/check', {
      headers: {
...
export const signup = async ({ identifier, role }: SignupPayload) => {
  ...
  try {
    const response = await fetch('http://10.171.84.121:5000/api/v0/auth/signup', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
export const verifySignup = async ({
  ...
  try {
    const response = await fetch('http://10.171.84.121:5000/api/v0/auth/verify-signup', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
export const login = async ({ identifier }: LoginPayload) => {
  ...
  return await fetchHandler(
    'http://10.171.84.121:5000/api/v0/auth/send-login-otp',
    {
      method: 'POST',
...
export const verifyLogin = async ({ identifier, otp }: VerifyLoginPayload) => {
  ...
  const data = await fetchHandler(
    'http://10.171.84.121:5000/api/v0/auth/verify-login',
    {
      method: 'POST',
      body: JSON.stringify({
        identifier: formattedIdentifier,
...
export const resendOtp = async ({ identifier }: ResendOtpPayload) => {
  ...
  return await fetchHandler('http://10.171.84.121:5000/api/v0/auth/resend-otp', {
    method: 'POST',
    body: JSON.stringify({ identifier: formattedIdentifier }),
  });
};
...
export const switchAccount = async ({ targetUserId }: SwitchAccountPayload) => {
  ...
  const data = await fetchHandler(
    'http://10.171.84.121:5000/api/v0/auth/switch-account',
    {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${token}`,
...
export const addLinkedAccount = async (payload: AddLinkedAccountPayload) => {
  ...
  const data = await fetchHandler(
    'http://10.171.84.121:5000/api/v0/auth/add-linked-account',
    {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${token}`,
...
export const fetchLinkedAccounts = async () => {
  ...
  const data = await fetchHandler(
    'http://10.171.84.121:5000/api/v0/auth/linked-accounts',
    {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${token}`,
...
export const removeLinkedAccount = async (targetUserId: string) => {
  ...
  const data = await fetchHandler(
    'http://10.171.84.121:5000/api/v0/auth/remove-linked-account',
    {
      method: 'DELETE',
      headers: {
        Authorization: `Bearer ${token}`,
...
export const checkAuthStatus = async () => {
  try {
    ...
    const response = await axios.get(
      'http://10.171.84.121:5000/api/v0/auth/check-auth',
      {
